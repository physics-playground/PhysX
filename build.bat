...
cmake -S "%PHYSX_ROOT_DIR%/compiler/public" -B "%PHYSX_SOURCE_ROOT_DIR%\build" -Ax64 -DTARGET_BUILD_PLATFORM=windows -DPX_OUTPUT_ARCH=x86 --no-warn-unused-cli -DCMAKEMODULES_PATH=%PM_CMAKEMODULES_PATH% -DCMAKE_PREFIX_PATH="%PHYSX_ROOT_DIR%/../externals/CMakeModules;%PHYSX_ROOT_DIR%/../externals/targa" -DPHYSX_ROOT_DIR="%PHYSX_ROOT_DIR%" -DPX_OUTPUT_LIB_DIR="%PHYSX_ROOT_DIR%" -DPX_OUTPUT_BIN_DIR="%PHYSX_ROOT_DIR%" -DPX_BUILDSNIPPETS=FALSE -DPX_BUILDPUBLICSAMPLES=FALSE -DPX_GENERATE_STATIC_LIBRARIES=FALSE -DNV_USE_STATIC_WINCRT=TRUE -DNV_USE_DEBUG_WINCRT=TRUE -DPX_FLOAT_POINT_PRECISE_MATH=FALSE -DCMAKE_INSTALL_PREFIX="%PHYSX_ROOT_DIR%/install/vc17win64/PhysX"
